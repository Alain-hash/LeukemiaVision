...
include("doctor_backend/update_appointment_status.php");
...
<body>
    <div class="container-fluid p-0">
        ...
        <div class="row g-0">
            ...
            //get the patient information for logged in doctor
            $Patients = array();
            if ($doctor_id) {
...
ORDER BY appointment.App_Date ASC;
            ...
            foreach ($Patients as $Patient) {
                $App_ID = $Patient['Appointment_ID'];
            }


            $stmt3 = $connection->prepare(
                "SELECT 1 FROM leukemia_test WHERE Appointment_ID = ? LIMIT 1"
            );
            ...
            <div class="col-md-9 col-lg-10 p-0 main-content">
                <div class="container-fluid">
                    ...
                    if (isset($_SESSION['success']) && !empty($_SESSION['success'])) {
                     ...
                     <div class="message-content">' . $_SESSION['success'] . '</div>
                    ...
                    <div class="row p-3 mb-4 bg-white shadow-sm">
                        <h4 class="mb-0">Patient Appointments</h4>
                    </div>
                    ...
                    <div class="card mb-4 instructions-card">
                        <div class="card-body">
                            ...
                            <p class="card-text">
                                Below is a list of all your appointments with your patients. Please note:
                            </p>
                            <ul>
                                <li>Only <strong>completed</strong> appointments can have reports created for them.</li>
                                <li>To create a report for a completed appointment, click on the <i class="bi bi-clipboard-check"></i> "Set Result" button in the Actions column.</li>
                                ...
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        <?php foreach ($Patients as $Patient) {
                                            ...
                                            // Check if report is already submitted 
                                            $App_ID = $Patient['Appointment_ID'];
                                            $stmt3 = $connection->prepare("SELECT 1 FROM leukemia_test WHERE Appointment_ID = ? LIMIT 1");
                                            $stmt3->bind_param("i", $App_ID);
                                            $stmt3->execute();
                                            $result3 = $stmt3->get_result();
                                            $reportExists = $result3->num_rows > 0;
                                        ...
                                        ?>
                                            ...
                                            <tr>
                                                ...
                                                if ($Patient['Status'] == 'Cancelled') { ?>
                                                    ...
                                                    <td>
                                                        <button class="btn btn-sm btn-outline-primary" disabled title="Result Already Set">Cannot make report</i></button>
                                                    </td>
                                                <?php } elseif ($Patient['Status'] == 'Scheduled' || $Patient['Status'] == 'Missed' ||  $Patient['Status'] == 'Canceled'  ) { ?>
                                                    <td><span class="badge status-scheduled px-3 py-2"><?= $Patient['Status'] ?></span></td>
                                                    <td>
                                                        <button class="btn btn-sm btn-outline-primary" disabled title="Result Already Set">cannot make report</i></button>
                                                    </td>
                                                    <?php } elseif ($Patient['Status'] == 'Completed') {
                                                    // Check if report exists for this specific appointment
                                                    if (!$reportExists) { ?>
                                                        <td><span class="badge status-completed px-3 py-2"><?= $Patient['Status'] ?></span></td>
                                                        <?php if ($Patient['Type'] == 'Treatment') { ?>
                                                            <td>
                                                                <a href="treatment_report.php?patient_id=<?= $Patient['User_ID'] ?>&patient_name=<?= urlencode($Patient['Name']) ?>&app_id=<?= $Patient['Appointment_ID'] ?>">
                                                                    <button class="btn btn-sm btn-outline-primary" title="Set Result">
                                                                        make report
                                                                    </button>
                                                                </a>
                                                            </td>
                                                        <?php } else { ?>
                                                            <td>
                                                                <a href="test_report.php?patient_id=<?= $Patient['User_ID'] ?>&patient_name=<?= urlencode($Patient['Name']) ?>&app_id=<?= $Patient['Appointment_ID'] ?>">
                                                                    <button class="btn btn-sm btn-outline-primary" title="Set Result">
                                                    ...
                                                    <?php } else { ?>
                                                        <td><span class="badge status-completed px-3 py-2"><?= $Patient['Status'] ?></span></td>
                                                        <td>
                                                            <button class="btn btn-sm btn-outline-primary" disabled title="Result Already Set">Result Already Set</i></button>
                                                        </td>
                                                    <?php } ?>
                                                <?php } ?>
                                            </tr>
