...
$sql1 = "SELECT Doctor_ID from doctor WHERE User_ID = ?";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['profileImage'])) {
    ...
    if ($_FILES['profileImage']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileType, $allowedTypes)) {
            $newFileName = uniqid('doctor_profile_', true) . '.' . $fileType;
            ...
            if (move_uploaded_file($_FILES['profileImage']['tmp_name'], $filePath)) {
                ...
                // Update the database - use Profile_Picture for doctor table
                $stmt = $connection->prepare("UPDATE doctor SET Profile_Picture = ? WHERE Doctor_ID = ?");
